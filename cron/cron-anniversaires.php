...
// Check to see if a user exists with this e-mail
        $reponse = $bdd->query('
        SELECT usersfriends.nom usersfriendsname, usersfriends.prenom usersfriendsprenom, users.email usersemail, usersfriends.datedenaissance usersfriendsdate, usersfriends.ID usersfriendsid, usersfriends.notifmail usersfriendsnotif, users.pseudo pseudo, usersfriends.iduser iduser, users.notifanniversaire notifanniversaire
        FROM usersfriends
        INNER JOIN users
        ...
        while ($donnees = $reponse->fetch())
        {
                ...
                if ((date('m-d', time())) == (substr($donnees['usersfriendsdate'], -5)) AND $donnees['usersfriendsnotif'] == 0 AND $donnees['notifanniversaire'] == 0) {
                ...
                if ((date('m-d', time())) == date("m-d", strtotime("-10 days", strtotime($donnees['usersfriendsdate'])))  AND $donnees['usersfriendsnotif'] == 0 AND $donnees['notifanniversaire'] == 0) {
                ...
                if ((date('m-d', time())) == date("m-d", strtotime("-20 days", strtotime($donnees['usersfriendsdate'])))  AND $donnees['usersfriendsnotif'] == 0 AND $donnees['notifanniversaire'] == 0) {
